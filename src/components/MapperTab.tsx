    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="bg-[var(--bg-card)] border border-[var(--border-primary)] rounded-xl overflow-hidden">
        <div className="flex items-center justify-between px-6 py-3 border-b border-[var(--border-primary)]">
          <h3 className="text-[var(--text-primary)] text-base">Mapper</h3>
          <div className="flex items-center gap-2">
            <button className="bg-[var(--hover-item)] border border-[var(--border-secondary)] rounded-md flex items-center gap-2 px-3 py-1.5 text-[var(--text-secondary)] text-xs hover:bg-[var(--hover-item)] transition-colors">
            ...
            <button
              onClick={() => setIsModalOpen(true)}
              className="bg-[var(--hover-item)] border border-[var(--border-secondary)] rounded-md flex items-center gap-2 px-3 py-1.5 text-[var(--text-secondary)] text-xs hover:bg-[var(--hover-item)] transition-colors"
            >
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
        ...
        <div className="grid grid-cols-[50px_1fr_1.5fr_1fr_120px] border-b border-[var(--border-primary)]">
          ...
          <div className="px-6 py-3"><span className="text-[var(--text-label)] text-xs font-medium">Source</span></div>
          <div className="px-6 py-3"><span className="text-[var(--text-label)] text-xs font-medium">Tactic</span></div>
        </div>
        ...
        {campaigns.map((campaign, idx) => (
          <div key={idx} className="grid grid-cols-[50px_1fr_1.5fr_1fr_120px] border-b border-[var(--border-subtle)] hover:bg-[var(--hover-bg)] transition-colors">
            ...
            <div className="px-6 py-2 flex items-center">
              <span className="text-[var(--text-primary)] text-xs truncate">{campaign.name}</span>
            </div>
            <div className="px-6 py-2 flex items-center">
              <span className="text-[var(--text-primary)] text-xs truncate">{campaign.source}</span>
            </div>
            <div className="px-6 py-2 flex items-center">
              {campaign.tactic ? (
                <span className="text-[#00bc7d] text-xs">{campaign.tactic}</span>
              ) : (
                <span className="text-[var(--text-label)] text-xs italic">(not set)</span>
              )}
            </div>
          </div>
        ))}
      ...
      <div className="flex flex-col gap-3">
        ...
        {existingRules.map((rule, idx) => (
          <div key={idx} className="bg-[var(--bg-card)] border border-[var(--border-primary)] rounded-xl p-4 hover:border-[var(--border-secondary)] transition-colors">
            <div className="flex items-center justify-between mb-1.5">
              <p className="text-[var(--text-primary)] text-sm font-medium">{rule.name}</p>
              <div className="flex items-center gap-1">
                <button className="w-6 h-6 rounded flex items-center justify-center hover:bg-[var(--hover-item)] text-[var(--text-label)] hover:text-[var(--text-primary)] transition-colors" title="Edit">
                ...
                <button className="w-6 h-6 rounded flex items-center justify-center hover:bg-[var(--hover-item)] text-[var(--text-label)] hover:text-[#ff2056] transition-colors" title="Delete">
