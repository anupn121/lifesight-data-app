...
export default function AIInsightCard({ insight, confidence, recommendations, children }: AIInsightCardProps) {
  ...
  return (
    <div className="flex flex-col gap-3 pt-3">
      {/* AI Insight bar */}
      <div className="bg-[#027b8e]/5 border-l-2 border-l-[#027b8e] rounded-r-lg px-3 py-2.5">
        <div className="flex items-start gap-2">
          {/* Sparkle icon */}
          <svg width="14" height="14" viewBox="0 0 16 16" fill="none" className="mt-0.5 shrink-0">
            <path d="M8 1L9.5 6.5L15 8L9.5 9.5L8 15L6.5 9.5L1 8L6.5 6.5L8 1Z" fill="#027b8e" opacity="0.7" />
          </svg>
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              <span className="text-[10px] font-semibold text-[#027b8e] uppercase tracking-wider">AI Insight</span>
        ...
        {recommendations && recommendations.length > 0 && (
          <div className="mt-2">
            <button
              ...
              className="flex items-center gap-1 text-[10px] text-[#027b8e] hover:underline font-medium"
            >
              <svg
                width="8" height="8" viewBox="0 0 8 8" fill="none"
                className={`transition-transform ${showRecs ? "rotate-90" : ""}`}
              >
                <path d="M2 1L6 4L2 7" stroke="#027b8e" strokeWidth="1.2" fill="none" strokeLinecap="round" />
              </svg>
              ...
              <ul className="mt-1.5 space-y-1 pl-4">
                {recommendations.map((rec, i) => (
                  <li key={i} className="text-[11px] text-[var(--text-muted)] leading-relaxed flex items-start gap-1.5">
                    <span className="text-[#027b8e] mt-0.5">•</span>
                    {rec}
                  </li>
                ))}
              </ul>
